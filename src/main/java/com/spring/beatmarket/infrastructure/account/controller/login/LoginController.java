...
import com.spring.beatmarket.infrastructure.account.controller.shared.AccountMessageResponse;
...
class LoginController {
    ...
    @PostMapping("/login")
    public ResponseEntity<AccountMessageResponse> authenticateAndGenerateToken(@RequestBody LoginRequestDto dto, HttpServletResponse response) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(new AccountMessageResponse("Invalid email or password."));
        }

        return ResponseEntity.ok(new AccountMessageResponse("Login successful."));
    }
    ...
    @PostMapping("/logout")
    public ResponseEntity<AccountMessageResponse> clearJwtAuthorizationCookie(HttpServletResponse response) {
        Cookie cookie = cookieService.clearTokenCookie();
        response.addCookie(cookie);
        return ResponseEntity.ok(new AccountMessageResponse("Logged out successfully."));
    }
